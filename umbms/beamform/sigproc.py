...
def crop_by_freq(fd_sinos, ini_f, fin_f, tar_ini_f, tar_fin_f):
    """Crops freq-domain sinograms by frequency

    Parameters
    ----------
    fd_sinos : array_like
        Frequency-domain sinograms, NxMxL array, where N is number of
        sinograms, M is number of frequency points, and L is number
        of antenna positions
    ini_f : float
        Initial frequency of fd_sino, in GHz
    fin_f : float
        Final frequency of fd_sino, in GHz
    tar_ini_f : float
        Target initial frequency to crop to, i.e., minimum frequency
        to keep, in GHz
    tar_fin_f : float
        Target final frequency to crop to, i.e., maximum frequency
        to keep, in GHz

    Returns
    -------
    cropped_fd_sinos : array_like
        The fd_sinograms after cropping
    """

    # Frequencies used in scan
    fd_freqs = np.linspace(ini_f, fin_f, np.size(fd_sinos, axis=1))

    # Get the target indices to keep when cropping
    tar_idxs = np.logical_and(tar_fin_f >= fd_freqs, fd_freqs >= tar_ini_f)

    # Crop signals to retain only frequencies between tar_ini_f and
    # tar_fin_f
    cropped_fd_sinos = fd_sinos[:, tar_idxs, :]

    return cropped_fd_sinos
